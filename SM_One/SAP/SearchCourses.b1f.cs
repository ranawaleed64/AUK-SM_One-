...
namespace SM_One
{
    [FormAttribute("SM_One.SearchCourses", "SAP/SearchCourses.b1f")]
    class SearchCourses : UserFormBase
    {
        ...
        public SearchCourses(string scholarshipCode, string subjectCodes, Matrix matrix0, DBDataSource rowsDb, SAPbouiCOM.Form form, string cardCode, Registration regForm)
        {
        ...
        {
            string Query = "Select T0.\"ItemCode\" as \"Course Code\",T0.\"ItemName\" as \"Course Name\",T0.\"U_Credits\" as \"Credit Hours\",T1.\"Price\" from OITM T0 inner join ITM1 T1 on T0.\"ItemCode\" = T1.\"ItemCode\" inner join OPLN T2 on T2.\"ListNum\" = T1.\"PriceList\" inner join \"@OSHL\" T3 on T3.\"U_PriceList\" = T2.\"ListName\" where T3.\"Code\" = '"+ ScholarshipCode + "'";
        ...
        private void btnChoose_PressedAfter(object sboObject, SBOItemEventArg pVal)
        {
            try
            {
                ...
                for (int i = 0; i < Grid0.Rows.SelectedRows.Count; i++)
                {
                    ...
                    Matrix0.SetCellWithoutValidation(Matrix0.RowCount, "SubCode", Grid0.DataTable.GetValue("Course Code", RowIndex).ToString());
                    Matrix0.SetCellWithoutValidation(Matrix0.RowCount, "SubName", Grid0.DataTable.GetValue("Course Name", RowIndex).ToString());
                    double Price = 0;
                    double CH = 0;
                    ...
                    Matrix0.SetCellWithoutValidation(Matrix0.RowCount, "Price", (Price).ToString());
                    Matrix0.SetCellWithoutValidation(Matrix0.RowCount, "Total", (Price * CH).ToString());
                ...
                UIAPIRawForm.Close();
                registration.CalculateDiscounts("");
